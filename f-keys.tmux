...
bind-key -n C-n new-session \; rename-window "-"
bind-key -n C-t new-window -c "#{pane_current_path}" \; rename-window "-"
bind-key -n C-d command-prompt -p "(rename-window) " "rename-window '%%'"
bind-key -n C-y command-prompt -p "(rename-session) " "rename-session '%%'"
